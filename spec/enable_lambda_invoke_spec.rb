...
describe 'compiled component aurora-postgres' do
  ...
  context 'cftest' do
    it 'compiles test' do
      expect(system("cfhighlander cftest #{@validate} --tests tests/enable_lambda_invoke.test.yaml")).to be_truthy
    end      
  end
  
  let(:template) { YAML.load_file("#{File.dirname(__FILE__)}/../out/tests/enable_lambda_invoke/aurora-postgres.compiled.yaml") }
  
  context "Resource" do

    
    context "AuroraLambdaInvokeIAMRole" do
      let(:resource) { template["Resources"]["AuroraLambdaInvokeIAMRole"] }
      ...
      it "to have property Policies" do
          expect(resource["Properties"]["Policies"]).to eq([{"PolicyName"=>"invoke-lambda", "PolicyDocument"=>{"Statement"=>[{"Sid"=>"invokelambda0", "Action"=>["lambda:InvokeFunction"], "Resource"=>[{"Fn::Sub"=>"arn:aws:lambda:${AWS::Region}:${AWS::AccountId}:function/my-function"}, {"Fn::Sub"=>"arn:aws:lambda:${AWS::Region}:${AWS::AccountId}:function/${EnvironmentName}-function"}, {"Fn::Sub"=>"arn:aws:lambda:${AWS::Region}:${AWS::AccountId}:function/${MyFunction}"}], "Effect"=>"Allow"}]}}])
      end
    ...
    context "DBCluster" do
      ...
      it "to have property AssociatedRoles" do
          expect(resource["Properties"]["AssociatedRoles"]).to eq([{"FeatureName"=>"Lambda", "RoleArn"=>{"Fn::GetAtt"=>["AuroraLambdaInvokeIAMRole", "Arn"]}}])
      end
