...
describe 'compiled component aurora-postgres' do
  ...
  context 'cftest' do
    it 'compiles test' do
      expect(system("cfhighlander cftest #{@validate} --tests tests/database_name_parameter.test.yaml")).to be_truthy
    end      
  end
  
  let(:template) { YAML.load_file("#{File.dirname(__FILE__)}/../out/tests/database_name_parameter/aurora-postgres.compiled.yaml") }
  ...
  context "Resource" do
    ...
    context "DBCluster" do
      ...
      it "to have property DatabaseName" do
          expect(resource["Properties"]["DatabaseName"]).to eq({"Fn::Sub"=>"${DatabaseName}"})
      end
