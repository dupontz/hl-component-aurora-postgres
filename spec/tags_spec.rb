...
describe 'compiled component aurora-postgres' do
  ...
  let(:template) { YAML.load_file("#{File.dirname(__FILE__)}/../out/tests/tags/aurora-postgres.compiled.yaml") }
  ...
  context "Resource" do
    ...
    context "SecurityGroup" do
      ...
      it "to have property Tags" do
          expect(resource["Properties"]["Tags"]).to eq([{"Key"=>"Name", "Value"=>{"Fn::Sub"=>"${EnvironmentName}-aurora-postgres"}}, {"Key"=>"Environment", "Value"=>{"Ref"=>"EnvironmentName"}}, {"Key"=>"EnvironmentType", "Value"=>{"Ref"=>"EnvironmentType"}}, {"Key"=>"Name", "Value"=>{"Fn::Sub"=>"${EnvironmentName}-tag-test"}}, {"Key"=>"CostCenter", "Value"=>{"Fn::Sub"=>"test"}}])
      end
