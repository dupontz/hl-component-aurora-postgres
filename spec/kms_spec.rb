...
describe 'compiled component aurora-postgres' do
  ...
  context 'cftest' do
    it 'compiles test' do
      expect(system("cfhighlander cftest #{@validate} --tests tests/kms.test.yaml")).to be_truthy
    end      
  end
  
  let(:template) { YAML.load_file("#{File.dirname(__FILE__)}/../out/tests/kms/aurora-postgres.compiled.yaml") }
  ...
  context "Resource" do
    ...
    context "DBCluster" do
      ...
      it "to have property KmsKeyId" do
          expect(resource["Properties"]["KmsKeyId"]).to eq({"Ref"=>"KmsKeyId"})
      end
