...
describe 'compiled component aurora-postgres' do
  ...
  context 'cftest' do
    it 'compiles test' do
      expect(system("cfhighlander cftest #{@validate} --tests tests/database_parameters.test.yaml")).to be_truthy
    end      
  end
  
  let(:template) { YAML.load_file("#{File.dirname(__FILE__)}/../out/tests/database_parameters/aurora-postgres.compiled.yaml") }
  ...
  context "Resource" do
    ...
    context "DBClusterParameterGroup" do
      ...
      it "to have property Parameters" do
          expect(resource["Properties"]["Parameters"]).to eq({"timezone"=>"UTC", "ssl_min_protocol_version"=>"TLSv1.2"})
      end
    ...
    context "DBInstanceParameterGroup" do
      ...
      it "to have property Parameters" do
          expect(resource["Properties"]["Parameters"]).to eq({"rds.force_admin_logging_level"=>"info"})
      end
